...
int main(int, char**)
{
    ...
    char code = (char)-1;
    VideoWriter video("outcpp.avi", cv::VideoWriter::fourcc('M','J','P','G'), 2, Size(500,500));
    ...
    for(;;)
    {
        ...
        KF.transitionMatrix = (Mat_<float>(2, 2) << 1, 1, 0, 1); //F
        setIdentity(KF.measurementMatrix); // H
        setIdentity(KF.processNoiseCov, Scalar::all(1e-5)); //Q
        setIdentity(KF.measurementNoiseCov, Scalar::all(1e-1)); //Unknown in a real case
        setIdentity(KF.errorCovPost, Scalar::all(1));  //P
        randn(KF.statePost, Scalar::all(0), Scalar::all(0.1)); //X after correction
        for(;;)
        {
