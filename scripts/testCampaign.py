...
f = open("used_yaw.txt", "r")
content = f.read()
f.close()
f = open("used_yaw.txt", "a")
# Iterate through the video files
for video_file in video_files:
    video_path = os.path.join(folder_path, video_file)
    print(video_path)
    output_url = "rtsp://localhost:8554/in"
    if video_file not in content:
        video_splice = video_file.split("_")
        print(video_splice)
        pitch =video_splice[0]
        yaw=video_splice[1]
        altitude=video_splice[2]
        lane =video_splice[3].split(".")[0]
        print(lane)
        #if lane ==  study_lane:
            # execute testCampaign cpp to send the message
        cpp_command = ['./build/bin/testCampaign', '-p', pitch, '-y', yaw, "-a", altitude,"-l",lane]
